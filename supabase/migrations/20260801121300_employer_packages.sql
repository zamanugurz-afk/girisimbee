-- P4: Employer package catalog, entitlements, coupons + payment purpose

DO $$ BEGIN
  ALTER TYPE public.marketplace_payment_purpose ADD VALUE IF NOT EXISTS 'employer_package';
...
CREATE UNIQUE INDEX IF NOT EXISTS employer_packages_slug_unique
  ON public.employer_packages (slug);

INSERT INTO public.employer_packages (
...
FROM (VALUES
  ...
  featured_listing, urgent_listing, homepage_visibility, badge_visibility, active_status, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM public.employer_packages ep WHERE ep.slug = v.slug);

CREATE TABLE IF NOT EXISTS public.employer_user_packages (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  package_slug TEXT NOT NULL REFERENCES public.employer_packages(slug),
...
CREATE INDEX IF NOT EXISTS employer_user_packages_user_id_idx
  ON public.employer_user_packages (user_id);
CREATE INDEX IF NOT EXISTS employer_user_packages_status_idx
  ON public.employer_user_packages (status);

CREATE TABLE IF NOT EXISTS public.employer_coupons (
...
INSERT INTO public.employer_coupons (code, discount_percent, valid_package_slugs, active)
SELECT v.code, v.discount_percent, v.valid_package_slugs, v.active
FROM (VALUES
  ('EMPLOYER10', 10, NULL::TEXT[], true),
  ('PRO15', 15, ARRAY['professional']::TEXT[], true)
) AS v(code, discount_percent, valid_package_slugs, active)
WHERE NOT EXISTS (SELECT 1 FROM public.employer_coupons c WHERE c.code = v.code);

ALTER TABLE public.employer_packages ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.employer_user_packages ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.employer_coupons ENABLE ROW LEVEL SECURITY;

CREATE POLICY employer_packages_public_read ON public.employer_packages
  FOR SELECT TO anon, authenticated USING (active_status = true);

CREATE POLICY employer_packages_admin_write ON public.employer_packages
  FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY employer_user_packages_owner_read ON public.employer_user_packages
  FOR SELECT TO authenticated USING (user_id = auth.uid());

CREATE POLICY employer_user_packages_admin_all ON public.employer_user_packages
  FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY employer_coupons_public_read ON public.employer_coupons
  FOR SELECT TO anon, authenticated USING (active = true);

DROP TRIGGER IF EXISTS employer_packages_updated_at ON public.employer_packages;
CREATE TRIGGER employer_packages_updated_at
  BEFORE UPDATE ON public.employer_packages
  FOR EACH ROW EXECUTE FUNCTION public.set_marketplace_updated_at();

DROP TRIGGER IF EXISTS employer_user_packages_updated_at ON public.employer_user_packages;
CREATE TRIGGER employer_user_packages_updated_at
  BEFORE UPDATE ON public.employer_user_packages
  FOR EACH ROW EXECUTE FUNCTION public.set_marketplace_updated_at();
