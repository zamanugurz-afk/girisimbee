...
import {
  entrepreneurPaymentActionSchema,
  entrepreneurPaymentListQuerySchema,
} from '@/lib/api/validation/entrepreneur-monetization';
import { ids } from '@/lib/domain/ids';

/** GET — entrepreneur payment history */
export const GET = withAuth(async (ctx, request) => {
  const url = new URL(request.url);
  const query = entrepreneurPaymentListQuerySchema.parse(Object.fromEntries(url.searchParams));
...
export const POST = withAuth(async (ctx, request) => {
  ...
  const parsed = entrepreneurPaymentActionSchema.parse(body);
  const service = ctx.container.ecosystem.entrepreneurMonetizationService;
