...
import {
  candidatePaymentActionSchema,
  candidatePaymentListQuerySchema,
...
export const GET = withAuth(async (ctx, request) => {
  ...
  const query = candidatePaymentListQuerySchema.parse(Object.fromEntries(url.searchParams));
...
export const POST = withAuth(async (ctx, request) => {
  ...
  const parsed = candidatePaymentActionSchema.parse(body);
  const service = ctx.container.ecosystem.candidateMonetizationService;
