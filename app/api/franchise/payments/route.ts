...
import {
  ...
  franchisePaymentListQuerySchema,
} from '@/lib/api/validation/franchise-monetization';
import { ids } from '@/lib/domain/ids';

/** GET — franchise payment history (own; admin may filter by userId) */
export const GET = withAuth(async (ctx, request) => {
  const url = new URL(request.url);
  const query = franchisePaymentListQuerySchema.parse(Object.fromEntries(url.searchParams));
...
export const POST = withAuth(async (ctx, request) => {
  ...
  const parsed = franchisePaymentActionSchema.parse(body);
  const service = ctx.container.ecosystem.franchiseMonetizationService;
