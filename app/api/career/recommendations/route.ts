...
import { assertNoContactLeak } from '@/features/matching-engine/adapters/public-card';
...
export async function GET(request: Request) {
  try {
    ...
    if (section) {
      assertNoContactLeak(section);
    }

    return NextResponse.json({ section });
  } catch (error) {
    console.error('Error fetching career recommendations for listing:', error);
