...
import {
  investorPaymentActionSchema,
  investorPaymentListQuerySchema,
...
export const GET = withAuth(async (ctx, request) => {
  ...
  const query = investorPaymentListQuerySchema.parse(Object.fromEntries(url.searchParams));
...
export const POST = withAuth(async (ctx, request) => {
  ...
  const parsed = investorPaymentActionSchema.parse(body);
  const service = ctx.container.ecosystem.investorMonetizationService;
