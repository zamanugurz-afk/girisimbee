...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!data || data.flow !== 'buy') {
    return { title: 'İlan Bulunamadı — Girisimco' };
  }

  return {
    title: `${data.listing.title} — Bayilik Ver | Girisimco`,
    description: data.listing.shortDescription,
  };
}

export default async function FranchiseGiveDetailPage({ params }: PageProps) {
  const data = await loadDetail(params.slug, true);
  if (!data || data.flow !== 'buy' || data.listing.status !== 'published') notFound();

  return (
    <FranchiseListingDetailView
      data={data}
      backHref="/franchise/give"
      backLabel="Bayilik Ver"
    />
  );
}
