...
const TURKISH_BROKEN_WORD_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/(?<=^|[^\p{L}\p{N}])[eE][ğg][\s\uFFFD\?]+[iİıI]tim(?:ler|i)?(?=$|[^\p{L}\p{N}])/giu, 'Eğitim'],
  [/(?<=^|[^\p{L}\p{N}])[dD][eE][\s\uFFFD\?]+[nN][eE][yY][iİıI]m(?:ler|i)?(?=$|[^\p{L}\p{N}])/giu, 'Deneyim'],
  [/(?<=^|[^\p{L}\p{N}])[iİıI][şsS][\s\uFFFD\?]+[dD][eE][nN][eE][yY][iİıI]m(?:ler|i)?(?=$|[^\p{L}\p{N}])/giu, 'İş Deneyimi'],
