/**
 * Supabase candidate package repository.
 */
...
const CATALOG_TABLE = 'candidate_packages';
const PACKAGES_TABLE = 'candidate_user_packages';
const COUPONS_TABLE = 'candidate_coupons';
...
function mapCatalog(row: CatalogRow): CandidatePackageCatalogItem {
  return {
    id: ids.candidatePackage(row.id),
    slug: row.slug as CandidatePackageSlug,
    packageName: row.package_name,
    packageType: row.package_type as CandidatePackageSlug,
...
function mapPackage(row: PackageRow): CandidateUserPackage {
  return {
    id: ids.candidatePackage(row.id),
    userId: ids.user(row.user_id),
    packageSlug: row.package_slug as CandidatePackageSlug,
    listingsUsed: row.listings_used,
    startsAt: row.starts_at,
    expiresAt: row.expires_at,
    status: row.status as CandidateUserPackageStatus,
    grantedBy: row.granted_by as CandidatePackageGrantSource,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

function mapCoupon(row: CouponRow): CandidateCoupon {
  return {
    code: row.code,
    discountPercent: row.discount_percent,
    discountCents: row.discount_cents,
    validPackageSlugs: row.valid_package_slugs as CandidatePackageSlug[] | null,
    active: row.active,
    expiresAt: row.expires_at,
  };
}

export class SupabaseCandidatePackageRepository implements CandidatePackageRepository {
  ...
  async grant(input: GrantCandidatePackageInput): Promise<CandidateUserPackage> {
    ...
    if (!catalog) throw new NotFoundError('CandidatePackage', input.packageSlug);
  ...
  async extendDuration(id: CandidatePackageId, extraDays: number): Promise<CandidateUserPackage> {
    ...
    if (!existing) throw new NotFoundError('CandidateUserPackage', id);
  ...
  listCoupons() {
    return listModuleCoupons(this.supabase, COUPONS_TABLE) as Promise<CandidateCoupon[]>;
  }

  upsertCoupon(input: CandidateCoupon) {
    return upsertModuleCoupon(this.supabase, COUPONS_TABLE, input) as Promise<CandidateCoupon>;
  }
