...
function computeDiscount(priceCents: number, coupon: InvestorCoupon): number {
...
export class InvestorMonetizationService {
  private readonly gateway: IyzicoGateway;

  constructor(
    private readonly packageRepo: InvestorPackageRepository,
  ...
  async getPackage(slug: InvestorPackageSlug) {
    const pkg = await this.packageRepo.getBySlug(slug);
    if (!pkg) throw new NotFoundError('InvestorPackage', slug);
    return pkg;
  }

  createCatalogItem(input: CreateInvestorCatalogInput) {
    return this.packageRepo.createCatalogItem(input);
  }

  async applyCoupon(code: string, packageSlug: InvestorPackageSlug): Promise<CouponApplyResult> {
  ...
  private async resolveCheckoutAmount(
  ...
  ): Promise<{ amountCents: number; metadata: InvestorPaymentMetadata }> {
    const catalog = await this.getPackage(packageSlug);
    ...
    return {
      amountCents,
      metadata: {
        investorPackageSlug: packageSlug,
        couponCode: code,
  ...
  async createCheckout(input: InvestorCheckoutInput) {
    ...
    const record = await this.paymentRepo.create({
      userId: input.userId,
      amountCents,
      purpose: 'investor_package',
      entityType: 'investor_package',
  ...
  async upgradePackage(
    userId: UserId,
    fromSlug: InvestorPackageSlug,
    toSlug: InvestorPackageSlug,
    urls: { successUrl: string; cancelUrl: string },
  ) {
    if (INVESTOR_PACKAGE_TIER[toSlug] <= INVESTOR_PACKAGE_TIER[fromSlug]) {
    ...
    if (!current) throw new NotFoundError('InvestorUserPackage', fromSlug);
  ...
  async renewPackage(
    ...
    if (!existing || existing.userId !== userId) {
      throw new NotFoundError('InvestorUserPackage', userPackageId);
    }
  ...
  async getPaymentHistory(userId: UserId) {
    const { data } = await this.paymentRepo.findMany({ userId, purpose: 'investor_package' });
    return data;
  }
  ...
  async getInvoice(paymentId: PaymentId) {
    ...
    if (payment.purpose !== 'investor_package') {
      throw new ForbiddenError('Bu ödeme yatırımcı paketi ile ilişkili değil.');
    }
    return {
      payment,
      invoice: payment.metadata as InvestorPaymentMetadata,
    };
  }

  async getRemainingDays(userPackageId: InvestorPackageId): Promise<number> {
    const pkg = await this.packageRepo.findById(userPackageId);
    if (!pkg) throw new NotFoundError('InvestorUserPackage', userPackageId);
    if (!pkg.expiresAt) return 0;
    const diff = new Date(pkg.expiresAt).getTime() - Date.now();
    return Math.max(0, Math.ceil(diff / 86400000));
  }

  async isActiveEntitlement(userPackageId: InvestorPackageId): Promise<boolean> {
    const pkg = await this.checkExpiration(userPackageId);
    return pkg.status === 'active';
  }

  async checkExpiration(userPackageId: InvestorPackageId): Promise<InvestorUserPackage> {
    const pkg = await this.packageRepo.findById(userPackageId);
    if (!pkg) throw new NotFoundError('InvestorUserPackage', userPackageId);
  ...
  activatePackage(userId: UserId, packageSlug: InvestorPackageSlug) {
    return this.packageRepo.grant({ userId, packageSlug, grantedBy: 'admin' });
  }

  suspendPackage(userPackageId: InvestorPackageId) {
    return this.packageRepo.updateStatus(userPackageId, 'suspended');
  }

  cancelPackage(userPackageId: InvestorPackageId) {
    return this.packageRepo.updateStatus(userPackageId, 'cancelled');
  }

  extendDuration(userPackageId: InvestorPackageId, extraDays: number) {
    if (extraDays <= 0) {
  ...
  async updatePayment(
    paymentId: PaymentId,
    input: { metadata?: InvestorPaymentMetadata; status?: MarketplacePayment['status'] },
  ) {
  ...
  async fulfillPayment(paymentId: PaymentId): Promise<InvestorUserPackage | null> {
    const payment = await this.paymentRepo.findById(paymentId);
    if (!payment || payment.purpose !== 'investor_package') return null;

    const meta = payment.metadata as InvestorPaymentMetadata;
    const slug = meta.toPackageSlug ?? meta.investorPackageSlug;
    if (!slug) return null;

    if (meta.action === 'renewal' && meta.userPackageId) {
      const catalog = await this.getPackage(slug);
      return this.packageRepo.extendDuration(meta.userPackageId as InvestorPackageId, catalog.packageDuration);
    }

    if (meta.action === 'upgrade' && meta.userPackageId) {
      await this.packageRepo.updateStatus(meta.userPackageId as InvestorPackageId, 'cancelled');
    }
