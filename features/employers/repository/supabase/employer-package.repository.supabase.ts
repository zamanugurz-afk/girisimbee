/**
 * Supabase employer package repository.
 */
...
const CATALOG_TABLE = 'employer_packages';
const PACKAGES_TABLE = 'employer_user_packages';
const COUPONS_TABLE = 'employer_coupons';
...
function mapCatalog(row: CatalogRow): EmployerPackageCatalogItem {
  return {
    id: ids.employerPackage(row.id),
    slug: row.slug as EmployerPackageSlug,
    packageName: row.package_name,
    packageType: row.package_type as EmployerPackageSlug,
...
function mapPackage(row: PackageRow): EmployerUserPackage {
  return {
    id: ids.employerPackage(row.id),
    userId: ids.user(row.user_id),
    packageSlug: row.package_slug as EmployerPackageSlug,
    listingsUsed: row.listings_used,
    startsAt: row.starts_at,
    expiresAt: row.expires_at,
    status: row.status as EmployerUserPackageStatus,
    grantedBy: row.granted_by as EmployerPackageGrantSource,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

function mapCoupon(row: CouponRow): EmployerCoupon {
  return {
    code: row.code,
    discountPercent: row.discount_percent,
    discountCents: row.discount_cents,
    validPackageSlugs: row.valid_package_slugs as EmployerPackageSlug[] | null,
    active: row.active,
    expiresAt: row.expires_at,
  };
}

export class SupabaseEmployerPackageRepository implements EmployerPackageRepository {
  ...
  async grant(input: GrantEmployerPackageInput): Promise<EmployerUserPackage> {
    ...
    if (!catalog) throw new NotFoundError('EmployerPackage', input.packageSlug);
  ...
  async extendDuration(id: EmployerPackageId, extraDays: number): Promise<EmployerUserPackage> {
    ...
    if (!existing) throw new NotFoundError('EmployerUserPackage', id);
  ...
  listCoupons() {
    return listModuleCoupons(this.supabase, COUPONS_TABLE) as Promise<EmployerCoupon[]>;
  }

  upsertCoupon(input: EmployerCoupon) {
    return upsertModuleCoupon(this.supabase, COUPONS_TABLE, input) as Promise<EmployerCoupon>;
  }
