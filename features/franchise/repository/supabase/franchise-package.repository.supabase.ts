/**
 * Supabase franchise package repository.
 */
...
const CATALOG_TABLE = 'franchise_packages';
const PACKAGES_TABLE = 'franchise_user_packages';
const COUPONS_TABLE = 'franchise_coupons';
...
function mapCatalog(row: CatalogRow): FranchisePackageCatalogItem {
  return {
    id: ids.franchisePackage(row.id),
    slug: row.slug as FranchisePackageSlug,
    packageName: row.package_name,
    packageType: row.package_type as FranchisePackageSlug,
...
function mapPackage(row: PackageRow): FranchiseUserPackage {
  return {
    id: ids.franchisePackage(row.id),
    userId: ids.user(row.user_id),
    packageSlug: row.package_slug as FranchisePackageSlug,
    listingsUsed: row.listings_used,
    startsAt: row.starts_at,
    expiresAt: row.expires_at,
    status: row.status as FranchiseUserPackageStatus,
    grantedBy: row.granted_by as FranchisePackageGrantSource,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

function mapCoupon(row: CouponRow): FranchiseCoupon {
  return {
    code: row.code,
    discountPercent: row.discount_percent,
    discountCents: row.discount_cents,
    validPackageSlugs: row.valid_package_slugs as FranchisePackageSlug[] | null,
    active: row.active,
    expiresAt: row.expires_at,
  };
}

export class SupabaseFranchisePackageRepository implements FranchisePackageRepository {
  ...
  async grant(input: GrantFranchisePackageInput): Promise<FranchiseUserPackage> {
    ...
    if (!catalog) throw new NotFoundError('FranchisePackage', input.packageSlug);
  ...
  async extendDuration(id: FranchisePackageId, extraDays: number): Promise<FranchiseUserPackage> {
    ...
    if (!existing) throw new NotFoundError('FranchiseUserPackage', id);
  ...
  listCoupons() {
    return listModuleCoupons(this.supabase, COUPONS_TABLE) as Promise<FranchiseCoupon[]>;
  }

  upsertCoupon(input: FranchiseCoupon) {
    return upsertModuleCoupon(this.supabase, COUPONS_TABLE, input) as Promise<FranchiseCoupon>;
  }
