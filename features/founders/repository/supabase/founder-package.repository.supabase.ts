/**
 * Supabase founder package repository.
 */
...
const CATALOG_TABLE = 'founder_packages';
const PACKAGES_TABLE = 'founder_user_packages';
const COUPONS_TABLE = 'founder_coupons';
...
function mapCatalog(row: CatalogRow): FounderPackageCatalogItem {
  return {
    id: ids.founderPackage(row.id),
    slug: row.slug as FounderPackageSlug,
    packageName: row.package_name,
    packageType: row.package_type as FounderPackageSlug,
...
function mapPackage(row: PackageRow): FounderUserPackage {
  return {
    id: ids.founderPackage(row.id),
    userId: ids.user(row.user_id),
    packageSlug: row.package_slug as FounderPackageSlug,
    listingsUsed: row.listings_used,
    startsAt: row.starts_at,
    expiresAt: row.expires_at,
    status: row.status as FounderUserPackageStatus,
    grantedBy: row.granted_by as FounderPackageGrantSource,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

function mapCoupon(row: CouponRow): FounderCoupon {
  return {
    code: row.code,
    discountPercent: row.discount_percent,
    discountCents: row.discount_cents,
    validPackageSlugs: row.valid_package_slugs as FounderPackageSlug[] | null,
    active: row.active,
    expiresAt: row.expires_at,
  };
}

export class SupabaseFounderPackageRepository implements FounderPackageRepository {
  ...
  async grant(input: GrantFounderPackageInput): Promise<FounderUserPackage> {
    ...
    if (!catalog) throw new NotFoundError('FounderPackage', input.packageSlug);
  ...
  async extendDuration(id: FounderPackageId, extraDays: number): Promise<FounderUserPackage> {
    ...
    if (!existing) throw new NotFoundError('FounderUserPackage', id);
  ...
  listCoupons() {
    return listModuleCoupons(this.supabase, COUPONS_TABLE) as Promise<FounderCoupon[]>;
  }

  upsertCoupon(input: FounderCoupon) {
    return upsertModuleCoupon(this.supabase, COUPONS_TABLE, input) as Promise<FounderCoupon>;
  }
