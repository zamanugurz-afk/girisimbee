...
function computeDiscount(priceCents: number, coupon: FounderCoupon): number {
...
export class FounderMonetizationService {
  private readonly gateway: IyzicoGateway;

  constructor(
    private readonly packageRepo: FounderPackageRepository,
  ...
  async getPackage(slug: FounderPackageSlug) {
    const pkg = await this.packageRepo.getBySlug(slug);
    if (!pkg) throw new NotFoundError('FounderPackage', slug);
    return pkg;
  }

  createCatalogItem(input: CreateFounderCatalogInput) {
    return this.packageRepo.createCatalogItem(input);
  }

  async applyCoupon(code: string, packageSlug: FounderPackageSlug): Promise<CouponApplyResult> {
  ...
  private async resolveCheckoutAmount(
  ...
  ): Promise<{ amountCents: number; metadata: FounderPaymentMetadata }> {
    const catalog = await this.getPackage(packageSlug);
    ...
    return {
      amountCents,
      metadata: {
        founderPackageSlug: packageSlug,
        couponCode: code,
  ...
  async createCheckout(input: FounderCheckoutInput) {
    ...
    const record = await this.paymentRepo.create({
      userId: input.userId,
      amountCents,
      purpose: 'founder_package',
      entityType: 'founder_package',
  ...
  async upgradePackage(
    userId: UserId,
    fromSlug: FounderPackageSlug,
    toSlug: FounderPackageSlug,
    urls: { successUrl: string; cancelUrl: string },
  ) {
    if (FOUNDER_PACKAGE_TIER[toSlug] <= FOUNDER_PACKAGE_TIER[fromSlug]) {
    ...
    if (!current) throw new NotFoundError('FounderUserPackage', fromSlug);
  ...
  async renewPackage(
    ...
    if (!existing || existing.userId !== userId) {
      throw new NotFoundError('FounderUserPackage', userPackageId);
    }
  ...
  async getPaymentHistory(userId: UserId) {
    const { data } = await this.paymentRepo.findMany({ userId, purpose: 'founder_package' });
    return data;
  }
  ...
  async getInvoice(paymentId: PaymentId) {
    ...
    if (payment.purpose !== 'founder_package') {
      throw new ForbiddenError('Bu ödeme kurucu paketi ile ilişkili değil.');
    }
    return {
      payment,
      invoice: payment.metadata as FounderPaymentMetadata,
    };
  }

  async getRemainingDays(userPackageId: FounderPackageId): Promise<number> {
    const pkg = await this.packageRepo.findById(userPackageId);
    if (!pkg) throw new NotFoundError('FounderUserPackage', userPackageId);
    if (!pkg.expiresAt) return 0;
    const diff = new Date(pkg.expiresAt).getTime() - Date.now();
    return Math.max(0, Math.ceil(diff / 86400000));
  }

  async isActiveEntitlement(userPackageId: FounderPackageId): Promise<boolean> {
    const pkg = await this.checkExpiration(userPackageId);
    return pkg.status === 'active';
  }

  async checkExpiration(userPackageId: FounderPackageId): Promise<FounderUserPackage> {
    const pkg = await this.packageRepo.findById(userPackageId);
    if (!pkg) throw new NotFoundError('FounderUserPackage', userPackageId);
  ...
  activatePackage(userId: UserId, packageSlug: FounderPackageSlug) {
    return this.packageRepo.grant({ userId, packageSlug, grantedBy: 'admin' });
  }

  suspendPackage(userPackageId: FounderPackageId) {
    return this.packageRepo.updateStatus(userPackageId, 'suspended');
  }

  cancelPackage(userPackageId: FounderPackageId) {
    return this.packageRepo.updateStatus(userPackageId, 'cancelled');
  }

  extendDuration(userPackageId: FounderPackageId, extraDays: number) {
    if (extraDays <= 0) {
  ...
  async updatePayment(
    paymentId: PaymentId,
    input: { metadata?: FounderPaymentMetadata; status?: MarketplacePayment['status'] },
  ) {
  ...
  async fulfillPayment(paymentId: PaymentId): Promise<FounderUserPackage | null> {
    const payment = await this.paymentRepo.findById(paymentId);
    if (!payment || payment.purpose !== 'founder_package') return null;

    const meta = payment.metadata as FounderPaymentMetadata;
    const slug = meta.toPackageSlug ?? meta.founderPackageSlug;
    if (!slug) return null;

    if (meta.action === 'renewal' && meta.userPackageId) {
      const catalog = await this.getPackage(slug);
      return this.packageRepo.extendDuration(meta.userPackageId as FounderPackageId, catalog.packageDuration);
    }

    if (meta.action === 'upgrade' && meta.userPackageId) {
      await this.packageRepo.updateStatus(meta.userPackageId as FounderPackageId, 'cancelled');
    }
